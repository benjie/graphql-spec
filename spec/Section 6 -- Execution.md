...
If a query error is encountered during input coercion of variable values, then
...
- For each {variableDefinition} in {variableDefinitions}:
  ...
  - Otherwise if {variableType} is a Non-Nullable type, and either {hasValue} is
    not {true} or {value} is {null}, throw a query error.
  - Otherwise if {hasValue} is true:
    ...
    - Otherwise:
      - If {value} cannot be coerced according to the input coercion rules of
        {variableType}, throw a query error.
...
executing the mutation’s top level selection set on the mutation root object
...
Executing a subscription creates a persistent function on the server that maps
an underlying Source Stream to a returned Response Stream.
...
Note: {resultMap} is ordered by which fields appear first in the query. This is
explained in greater detail in the Field Collection section below.
...
See the [Errors and Non-Nullability](#sec-Errors-and-Non-Nullability) section of
Field Execution for more about this behavior.
...
- For each {argumentDefinition} in {argumentDefinitions}:
  ...
  - Otherwise if {argumentType} is a Non-Nullable type, and either {hasValue} is
    not {true} or {value} is {null}, throw a field error.
  - Otherwise if {hasValue} is true:
    - If {value} is {null}:
      - Add an entry to {coercedValues} named {argumentName} with the value
        {null}.
    ...
    - Otherwise:
      - If {value} cannot be coerced according to the input coercion rules of
        {argumentType}, throw a field error.
      - Let {coercedValue} be the result of coercing {value} according to the
...
the field returned {null}, and an error must be added to the {"errors"} list in
the response.

If the result of resolving a field is {null} (either because the function to
resolve the field returned {null} or because an error occurred), and that field
is of a `Non-Null` type, then a field error is thrown. The error must be added
to the {"errors"} list in the response.

If the field returns {null} because of an error which has already been added to
the {"errors"} list in the response, the {"errors"} list must not be further
affected. That is, only one error should be added to the errors list per field.
