...
To avoid ambiguity, a document containing {TypeSystemDefinition} is invalid for
execution.
...
leaf nodes of any GraphQL query.
...
or enum. Leaf selections on objects, interfaces, and unions without subfields
are disallowed.
...
selection set of the target fragment is unioned with the selection set at the
...
is consider valid because {Dog} implements interface {Pet} and is a member of
{DogOrHuman}.
...
define a referenced variable, the query is invalid.
...
\${intArg} typed as {Int} cannot be used as a argument to {booleanArg}, typed as
{Boolean}.
...
default value in the query. This behavior is explicitly supported for
